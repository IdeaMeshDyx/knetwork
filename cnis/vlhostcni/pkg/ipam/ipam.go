...
func (g *Get) NodeNames() ([]string, error) {
	...
	if err != nil {
		klog.Errorf("这里从 etcd 获取全部 nodes key 失败, err: ", err.Error())
		return nil, err
	}
...
func (g *Get) HostNetwork() (*Network, error) {
	...
	for _, link := range linkList {
		...
		if link.Type() == "device" {
			...
			if err != nil {
				klog.Fatalf("Failed to get IPv4 address for link %s: %v", link.Attrs().Name, err)
			}
			if len(addr) >= 1 {
...
func (g *Get) NodeIp(hostName string) (string, error) {
	...
	if err != nil {
		klog.Errorf("获取集群节点 ip 失败, err: ", err.Error())
		return "", err
	}
	...
	if err != nil {
		klog.Errorf("初始化正则表达式失败, err: ", err.Error())
		return "", nil
	}
	ip, err := r.FindStringMatch(val)
	if err != nil {
		klog.Errorf("正则匹配 ip 失败, err: ", err.Error())
		return "", nil
	}
